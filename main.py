...
def query_ollama(prompt, system_prompt=None):
    ...
    try:
        response = requests.post(ai_config["MODEL_API_URL"], json=data, stream=True)
    except Exception as e:
        print(f"AI Error: {e}")
    ...
    if response.status_code == 200:
        ...
        return f"{ai_config['ERROR_RESPONSE']}"
...
Keep responses concise and natural for Discord chat."""
                    ...
                    # Get AI response
                    ai_response = await asyncio.to_thread(query_ollama, prompt, system_prompt)
                    ...
                    if ai_response:
                        ...
                        update_conversation_memory(message.channel.id, message.author.name, clean_content, ai_response)
                    else:
